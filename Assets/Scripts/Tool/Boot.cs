    ...
    public class Boot : MonoBehaviour
    {
        ...
        IEnumerator LoadTest()
        {
            ...
            else if (PlayMode == EPlayMode.HostPlayMode)
            {
                ...
                initParameters.DefaultHostServer = "https://cdn-1257380158.cos.ap-nanjing.myqcloud.com/JohnLemon/WebGL/" + version;
        ...
        IEnumerator Download()
        {
            ...
            //sliderText.text = (downloader.Progress * 100).ToString();
        ...
        IEnumerator Start1()
        {
            ...
            SceneOperationHandle scene = YooAssets.LoadSceneAsync("MainScene", sceneMode, activateOnLoad);

            yield return scene;
            Debug.Log($"Prefab name is {scene}");
            //AssetOperationHandle handleText = package.LoadAssetAsync<TMP_FontAsset>("TMPChineseFont");
            //yield return handleText;

            //AssetOperationHandle handle = package.LoadAssetAsync<GameObject>("温湿度UI_Image");
            //yield return handle;
            //GameObject go = handle.InstantiateSync(GameObject.Find("Canvas").transform);
            //go.transform.localScale = Vector3.one * 3;
            //go.transform.GetChild(0).GetComponent<TMP_Text>().font = handleText.AssetObject as TMP_FontAsset;
        ...
        private void OnDownloadProgressUpdateFunction(int totalDownloadCount, int currentDownloadCount, long totalDownloadBytes, long currentDownloadBytes)
        {
            ...
            sliderText.text = ((float)((float)currentDownloadBytes / (float)totalDownloadBytes * 1.0f) * 100).ToString("00.0") + "%";
        }
...
#if UNITY_ANDROID && UNITY_EDITOR/// <summary>
